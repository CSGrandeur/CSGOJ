  ...
  <ul v-show="isShow" class="accept-queue">
    <TransitionGroup name="tg-accept-queue">
      <li v-for="i in queueList" :key="i.id">
        <div class="accept-queue__rank" :class="i.medal !== null ? 'accept-queue__rank--' + i.medal : ''">{{ i.rank }}</div>
        <div class="accept-queue__name"><span>{{ i.name }}</span></div>
        <div class="accept-queue__solved">{{ i.solved }}</div>
        <div class="accept-queue__problem" :style="'--bg-color:' + problemColor[i.problem]">{{ String.fromCharCode(i.problem + 'A'.charCodeAt(0)) }}</div>
        <div class="accept-queue__penalty" :class="i.first ? 'accept-queue__penalty--first' : ''">{{ i.penalty }}</div>
      </li>
    </TransitionGroup>
  </ul>
</Transition>
